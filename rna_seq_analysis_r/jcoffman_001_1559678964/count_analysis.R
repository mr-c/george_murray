...
zebrafish <- factor(c(1, 2, 3, 4, 5, 6))
treatment <- factor(c("CTRL","CTRL","1uM","1uM","1uM","CTRL"))

sample_names <- factor(c("CTRL_1","CTRL_2","CORT_1","CORT_2",
                      "CORT_3","CTRL_3"))

# Visualize how factors can be combined into a "design matrix"
data.frame(sample = colnames(DGE), zebrafish, treatment)

# Make design matrix
design <- model.matrix(~ 0 + zebrafish + treatment)
...
help(plotMDS)
# PCA
dge_matrix <- as.matrix(DGE)
project.pca <- prcomp(t(dge_matrix))
summary(project.pca)
...
'''install.packages("factoextra")
library(factoextra)
dge.pca <- prcomp(data, scale = TRUE)
fviz_eig(dge.pca)
fviz_pca_ind(dge.pca,
             col.ind = "cos2", # Color by the quality of representation
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
)
...
'''DGE.PCA <- prcomp(DGE, center = TRUE, scale. = TRUE)
summary(DGE.PCA)
str(DGE.PCA)
library(devtools)
